...
module.exports = require("@web3auth/base-plugin");
...
// EXTERNAL MODULE: external "@web3auth/base-plugin"
var base_plugin_ = __webpack_require__(3);
...
class core_Web3AuthCore extends openlogin_jrpc_["SafeEventEmitter"] {
  constructor(options) {
    ...
    if (options.enableLogging) base_["log"].enableAll();else base_["log"].disableAll();
    ...
    this.cachedAdapter = Object(base_["storageAvailable"])(this.storage) ? window[this.storage].getItem(ADAPTER_CACHE_KEY) : null;
  ...
  clearCache() {
    if (!Object(base_["storageAvailable"])(this.storage)) return;
  ...
  async getUserInfo() {
    base_["log"].debug("Getting user info", this.status, this.connectedAdapterName);
    if (this.status !== base_["ADAPTER_STATUS"].CONNECTED || !this.connectedAdapterName) throw base_["WalletLoginError"].notConnectedError("No wallet is connected");
  ...
  async addPlugin(plugin) {
    ...
    if (plugin.pluginNamespace !== base_plugin_["PLUGIN_NAMESPACES"].MULTICHAIN && plugin.pluginNamespace !== this.coreOptions.chainConfig.chainNamespace) throw new Error("This plugin belongs to ".concat(plugin.pluginNamespace, " namespace which is incompatible with currently used namespace: ").concat(this.coreOptions.chainConfig.chainNamespace));
  ...
  subscribeToAdapterEvents(walletAdapter) {
    walletAdapter.on(base_["ADAPTER_EVENTS"].CONNECTED, async data => {
      this.status = base_["ADAPTER_STATUS"].CONNECTED;
      this.connectedAdapterName = data.adapter;
      this.cacheWallet(data.adapter);
      base_["log"].debug("connected", this.status, this.connectedAdapterName);
    ...
    walletAdapter.on(base_["ADAPTER_EVENTS"].DISCONNECTED, async data => {
      // get back to ready state for rehydrating.
      this.status = base_["ADAPTER_STATUS"].READY;

      if (Object(base_["storageAvailable"])(this.storage)) {
      ...
      base_["log"].debug("disconnected", this.status, this.connectedAdapterName);
      await Promise.all(Object.values(this.plugins).map(plugin => {
    ...
    walletAdapter.on(base_["ADAPTER_EVENTS"].CONNECTING, data => {
      ...
      base_["log"].debug("connecting", this.status, this.connectedAdapterName);
    });
    walletAdapter.on(base_["ADAPTER_EVENTS"].ERRORED, data => {
      ...
      base_["log"].debug("errored", this.status, this.connectedAdapterName);
    });
    walletAdapter.on(base_["ADAPTER_EVENTS"].ADAPTER_DATA_UPDATED, data => {
      base_["log"].debug("adapter data updated", data);
