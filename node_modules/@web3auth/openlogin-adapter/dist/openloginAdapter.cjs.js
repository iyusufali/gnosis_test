...
const getOpenloginDefaultOptions = (chainNamespace, chainId) => {
  return {
    ...
    chainConfig: chainNamespace ? Object(base_["getChainConfig"])(chainNamespace, chainId) : null,
    loginSettings: {
      relogin: true
    }
  };
...
var base_provider_ = __webpack_require__(3);
...
class openloginAdapter_OpenloginAdapter extends base_["BaseAdapter"] {
  constructor(params) {
    ...
    base_["log"].debug("const openlogin adapter", params);
    ...
    if ((_params$chainConfig3 = params.chainConfig) !== null && _params$chainConfig3 !== void 0 && _params$chainConfig3.chainNamespace) {
      ...
      base_["log"].debug("const openlogin chainConfig", this.chainConfig);
  ...
  async init(options) {
    ...
    if (!this.chainConfig && this.currentChainNamespace !== base_["CHAIN_NAMESPACES"].OTHER) throw base_["WalletInitializationError"].invalidParams("chainConfig is required before initialization");
    let isRedirectResult = false;

    if (this.openloginOptions.uxMode === openlogin_["UX_MODE"].REDIRECT) {
    ...
    try {
      ...
      base_["log"].error("Failed to connect with cached openlogin provider", error);
  ...
  async connect(params) {
    ...
    try {
      ...
      base_["log"].error("Failed to connect with openlogin provider", error); // ready again to be connected
  ...
  async connectWithProvider(params) {
    ...
    } else if (this.currentChainNamespace === base_["CHAIN_NAMESPACES"].OTHER) {
      this.privKeyProvider = new base_provider_["CommonPrivateKeyProvider"]();
    } else {
