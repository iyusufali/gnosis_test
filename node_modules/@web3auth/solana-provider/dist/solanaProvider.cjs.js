...
// EXPORTS
__webpack_require__.d(__webpack_exports__, "PrivateKeyWallet", function() { return /* reexport */ PrivateKeyWallet_PrivateKeyWallet; });
...
var external_bs58_ = __webpack_require__(1);
var external_bs58_default = /*#__PURE__*/__webpack_require__.n(external_bs58_);

// CONCATENATED MODULE: ./src/PrivateKeyWallet.ts




class PrivateKeyWallet_PrivateKeyWallet {
  ...
  async signAndSendTransaction(transaction) {
    ...
    const sig = await this.provider.request({
      ...
      params: [external_bs58_default.a.encode(signedTx.serialize())]
    });
    return {
      signature: sig
    };
  ...
  async signTransaction(transaction) {
    ...
    const keyPair = web3_js_["Keypair"].fromSecretKey(Buffer.from(privateKey, "hex"));
  ...
  async signMessage(data) {
    ...
    const keyPair = web3_js_["Keypair"].fromSecretKey(Buffer.from(privateKey, "hex"));
    const signature = tweetnacl_js_default.a.sign.detached(data, keyPair.secretKey);
    return signature;
  }
...
function createConfigMiddleware(providerConfig) {
  ...
  const configMiddleware = Object(openlogin_jrpc_["mergeMiddleware"])([createChainIdMiddleware(chainId), createProviderConfigMiddleware(providerConfig)]);
...
function createJsonRpcClient(providerConfig) {
  ...
  const networkMiddleware = Object(openlogin_jrpc_["mergeMiddleware"])([createConfigMiddleware(providerConfig), fetchMiddleware]);
  return {
    networkMiddleware,
    fetchMiddleware
  };
...
async function getProviderHandlers(_ref) {
  ...
  const transactionGenerator = serializedTx => {
    ...
    const tx = web3_js_["Transaction"].from(decodedTx);
    return tx;
  };
  ...
  const providerHandlers = {
    ...
    getSecretKey: async () => external_bs58_default.a.encode(keyPair.secretKey),
    signTransaction: async req => {
